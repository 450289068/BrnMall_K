                    ...
                    if (_ipromotionnosqlstrategy == null)
                    {
                        lock (_locker)
                        {
                            if (_ipromotionnosqlstrategy == null)
                            {
                                ...
                                {
                                    string[] fileNameList = Directory.GetFiles(System.Web.HttpRuntime.BinDirectory, "BrnMall.NOSQLStrategy.*.dll", SearchOption.TopDirectoryOnly);
                                    _ipromotionnosqlstrategy = (IPromotionNOSQLStrategy)Activator.CreateInstance(Type.GetType(string.Format("BrnMall.NOSQLStrategy.{0}.PromotionNOSQLStrategy, BrnMall.NOSQLStrategy.{0}", fileNameList[0].Substring(fileNameList[0].LastIndexOf("NOSQLStrategy.") + 14).Replace(".dll", "")),
                                                                                                                                    false,
                                                                                                                                    true));
                                }
                                catch
                                {
                                    throw new BMAException("创建'促销活动非关系数据库策略对象'失败,可能存在的原因:未将'促销活动非关系数据库策略程序集'添加到bin目录中;'促销活动非关系数据库策略程序集'文件名不符合'BrnMall.NOSQLStrategy.{策略名称}.dll'格式");
                                }
                            }
                        }
                    }
